...
// Main function
void hcalAnalysisHist(int run_num, int kevents, int seg) {
    
    //Initialize Histograms
    hHcal_atime = new TH1D("hHcal_atime","pulse adc time; hcal.a_time [ns]; ",60,0,200.);
    hHcal_clus_adctime = new TH1D("hHcal_clus_adctime","HCal ADC Cluster Time; adctime [ns]; counts",500, 0, 200.);

    hHcal_atimeblk = new TH1D("hHcal_atimeblk","atime of block; hcal.atimeblk [ns]; counts",500,0,200.);
    ...
    while (tree.Next()) {
        ...
        for (int nElem = 0; nElem < N_adcElemId; ++nElem) {
           hHcal_atime->Fill(sbs_hcal_a_time[nElem]);
        }
        ...
        //Fill HCal cluster blk histograms
        Int_t N_idblk = sbs_hcal_idblk.GetSize();
        for (UInt_t nElem = 0; nElem < N_idblk; ++nElem){
            hHcal_atimeblk->Fill(sbs_hcal_atimeblk[nElem]);
        }
    }
...
auto plotBlks(){
    auto c2 = new TCanvas("c2", "HCal atimblk", 800, 800);
    c2->SetLogy();

    hHcal_atimeblk->SetLineColor(kBlue);
    hHcal_atimeblk->SetLineWidth(2);
    hHcal_atimeblk->SetFillColor(kBlue);
    hHcal_atimeblk->SetStats(true);
